...
class StopLambdaTestTunnel extends DefaultTask {
    ...
    void stop() {
        try {
            ...
            if (LambdaTestTunnel.userAPIPort) {
                delete = new URL(hostURL + LambdaTestTunnel.userAPIPort + apiClosure).openConnection()
            }
            else
            {
        ...
        {
            System.out.println("Issue in closing tunnel")
        }
    }
}
