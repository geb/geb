...
Geb ships with three implementations: `{page-source-reporter-api}`, `{screenshot-reporter-api}` and `{frames-source-reporter-api}`.
...
DOM state as HTML). If you wish to <<reporter-configuration, configure the reporting>> to also write reports which contain the source content of page frames you can do so the following way:

[source,groovy]
----
include::{test-dir}/configuration/ReporterConfigSpec.groovy[tag=frames_source_reporter_config,indent=0]
----
