...
It is possible to verify if a given `Navigator` object holds the currently focused element using its `focused` property.
`Navigator` objects that don't match any elements will return `false` as the value of `focused` property.
