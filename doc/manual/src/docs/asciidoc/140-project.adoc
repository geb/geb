...
* Change implementation of `withAlert()` and `withConfirm()` to use the native WebDriver way of handling alerts. [issue:243[]]

==== Breaking changes
* `withNoAlert()` and `withNoConfirm()` methods have been removed because they became obsolete as `{unhandled-alert-exception-api}` is now automatically thrown when an unexpected javascript dialog is shown
