...
* github-profile:mkutz[Michael Kutz] - Addition of `{number-input-api}`, `{url-input-api}`, `{password-input-api}`, `{color-input-api}`, `{datetime-local-input-api}`, `{time-input-api}` and `{month-input-api}`
