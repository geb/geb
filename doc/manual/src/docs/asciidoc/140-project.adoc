...
* github-profile:joschi[Jochen Schalanda] - Doc fixes
...
* Fix documentation around calling value() on unchecked checkboxes. [issues:520[]]
* Make additional capabilities passed to cloud driver factory's `create()` method override the capabilities that are hardcoded for a particular cloud driver provider. [issues:372[]]
* Fixed `getCheckedLabel()` on `RadioButtons` module to return label text instead of `null` when a checked radio button is wrapped by a label without a `for` attribute. [issues:530[]]
