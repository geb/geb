...
[source,groovy]
----
include::{test-dir}/javascript/RefreshWaitingSpec.groovy[tag=page,indent=0]

include::{test-dir}/javascript/RefreshWaitingSpec.groovy[tag=test,indent=0]
----
...
[IMPORTANT]
====
This section introduces a feature which depends on a Chrome specific, custom WebDriver command.
Calling the methods mentioned in the next paraghraph will only work when driving a local Chrome browser and will throw an exception for all other browsers, including a remote Chrome browser driven via an instance of `RemoteWebDrvier`.
====
...
[source,groovy]
----
include::{core-test-dir}/geb/BrowserNetworkLatencySpec.groovy[tag=setNetworkLatency,indent=0]
----
