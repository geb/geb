...
Browser objects have `{at-method-taking-class-api}` and `{at-method-taking-class-api}` methods that test whether or not the browser is currently at the page modeled by the given page
class or instance.
...
(e.g. by using link:api/geb/Browser.html#page(T)[`page(Page)`] or link:api/geb/Browser.html#at(Class<T>)[`at(Page)`]) then it will be reverted to its original value after returning from
`withWindow()`.
====
...
[NOTE]
====
If code of the closure passed as the last argument changes browser's current page instance (e.g. by using link:api/geb/Browser.html#page(T)[`page(Page)`] or
link:api/geb/Browser.html#at(Class%3CT%3E)[`at(Page)`]) then it will be reverted to its original value after returning from `withNewWindow()`.
====
