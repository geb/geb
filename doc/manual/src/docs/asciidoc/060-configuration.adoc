...
[WARNING]
====
Bear in mind that since Geb 6.0 one `Configuration` instance is shared between all of the tests so to avoid configuration runtime overrides bleeding from one test to another you will have to revert any overrides after the test like it is done in `cleanup()` of the above example.
====