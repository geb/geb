...
If for some reason you would like the driver not to be cached but quit after each test and recreated before the next then Geb supports such management of the driver instance.
To quit the driver after each test you can set the `quitDriverOnBrowserReset` config property to `true`.

[source,groovy]
----
include::{test-dir}/configuration/QuitDriverOnBrowserResetConfigSpec.groovy[tag=config,indent=0]
----

[NOTE]
====
If the driver is fully managed by Geb and being using the <<driver-factory-closure, configured driver factory closure>> then it probably makes sense to disable <<driver-caching-configuration, driver caching>> when setting `quitDriverOnBrowserReset` to `true`.
====
...
[source,groovy]
----
include::{test-dir}/configuration/TemplateOptionsConfigScriptSpec.groovy[tag=config,indent=0]
----
...
[source,groovy]
----
include::{test-dir}/configuration/RequirePageAtCheckersConfigSpec.groovy[tag=config,indent=0]
----
...
[source,groovy]
----
include::{test-dir}/configuration/WithWindowConfigScriptSpec.groovy[tag=config,indent=0]
----
...
[source,groovy]
----
include::{test-dir}/configuration/WithNewWindowConfigScriptSpec.groovy[tag=config,indent=0]
----
...
[source,groovy]
----
include::{test-dir}/configuration/AutoClearWebStorageConfigSpec.groovy[tag=config,indent=0]
----
