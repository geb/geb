...
If it is not found, Geb will try to load a `{config-slurper-api}` class named `GebConfig` from the _default package_ - this is usefull if you run tests that use Geb from an IDE because you won't have
...
[NOTE]
====
In a Grails project, the `test/functional` directory is a good place to put your config script in.
...
For example, Grails dictates what the base URL and directory for reports should be set to and the Geb plugin for Grails uses the build adapter mechanism to set this up.
...
If no reporter is explicitly defined, a link:api/geb/report/CompositeReporter.html[composite reporter] will be created from a `ScreenshotReporter` (takes a PNG screenshot) and `PageSourceReporter`
...
Certain integrations will automatically clear the driver's cookies, which is usually necessary when using an <<implicit-driver-lifecycle, implicit driver>>.
