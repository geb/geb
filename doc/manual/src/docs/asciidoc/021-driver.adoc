...
This can be done via the methods on the `WebDriver` instance (obtainable via `{browser-get-driver-api}`) or by calling the <<quiting-the-browser,delegating methods on the browser object>>.
...
This means that you may need to call the `{browser-clear-cookies-api}` method on the browser in order not to get strange results due to cookies from previous executions.

[TIP]
====
Some of the integrations (e.g. Spock, JUnit) automatically clear the browser cookies at appropriate times such as after each test.
...
See https://github.com/geb/issues/issues/98[this issue] for some tips on how to tune its logging.