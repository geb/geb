...
Geb ships with two implementations: `{page-source-reporter-api}` and `{screenshot-and-page-reporter-api}`.
...
Reporting listeners are of type `{reporting-listener}` can be specified as part of the config…
