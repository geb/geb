...
This can be set with a static `atCheckWaiting` property.
...
The possible values for the `atCheckWaiting` option are consistent with the <<content-dsl-wait,ones for `wait` option of content definitions>>.

The `atCheckWaiting` value configured at page level takes priority over the global value configured via <<at-check-waiting,configuration>>.
